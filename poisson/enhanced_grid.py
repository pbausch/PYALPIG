...
def slice_mul(slice1, slice2, length):
    if type(slice2) is int:
        ...
        if type(slice1) is type(Ellipsis):
            ...
            return slice(
                b, min(slice1.start + slice2.stop * slice1.step, slice1.stop), s
            )
...
class GridWindow2D(Container2D):
    def __init__(self, grid, x, y):
        ...
        Container2D.__init__(
            self, slice_len(self.x, grid.width), slice_len(self.y, grid.height)
        )
...
class GridSliceXZ(Container2D, AuxiliaryContainer):  # constant Y
    def __init__(self, grid, x, y, z):
        ...
        Container2D.__init__(
            self, slice_len(self.x, grid.width), slice_len(self.z, grid.depth)
        )
...
class GridSliceYZ(Container2D, AuxiliaryContainer):  # constant x
    def __init__(self, grid, x, y, z):
        ...
        Container2D.__init__(
            self, slice_len(self.y, grid.height), slice_len(self.z, grid.depth)
        )
...
class GridWindow3D(Container3D, AuxiliaryContainer):
    def __init__(self, grid, x, y, z):
        ...
        Container3D.__init__(
            self,
            slice_len(self.x, grid.width),
            slice_len(self.y, grid.height),
            slice_len(self.z, grid.depth),
        )
